...
def render_power_generation_page():
    """Render the UK Power Supply Monitor dashboard page."""
    ...
    if time_filter == "historical":
        ...
        with col1:
            start_date = st.date_input(
                ...
                key="power_gen_start_date"
            )
        with col2:
            end_date = st.date_input(
                "End Date",
                datetime.now() - timedelta(days=1),
                key="power_gen_end_date"
            )
    else:
    ...
    try:
        with st.spinner("Loading data..."):
            df_initial = get_power_data(
                time_filter, start_date, end_date, secrets)
        ...
        if time_filter == "recent":
            ...
            with col2:
                ...
                st.metric("Avg Carbon Intensity",
                          f"{avg_intensity:.0f} gCO2/kWh")
